(function receiptStore(){
 ...
 async function save(isPublic){
  ...
  const session=JSON.parse(sessionStorage.getItem('afri-account')||'null');
  config ||= await fetch('/community-config.json').then(r=>r.json());
 ...
 function start(){const actions=document.querySelector('.receipt-actions');if(!actions)return;const button=document.createElement('button');button.textContent='Publish share link';button.disabled=true;button.title='Publishes your receipt so anyone with its link can view it';actions.append(button);const account=document.createElement('a');account.href='/studio/board';account.textContent='Sign in / My saved receipts';actions.after(account);const result=document.createElement('p');result.setAttribute('role','status');account.after(result);
 window.addEventListener('afri:receipt-invalidated',()=>{latest=null;button.disabled=true;result.textContent='';});
 window.addEventListener('afri:receipt-generated',async event=>{latest=event.detail;button.disabled=false;result.textContent='';if(sessionStorage.getItem('afri-account'))try{await save(false);result.textContent='Saved privately to My board.';}catch(err){result.textContent=err.message;}});
